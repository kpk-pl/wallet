...
def interp(data:List[QuoteHistoryItem], timeScale:List[datetime], leftFill = None, rightFill = None):
    ...
    for dateIdx in timeScale:
        ...
        if quoteIdx == 0 and data[0].timestamp > dateIdx:
            ...
            result.append(QuoteHistoryItem.model_construct(timestamp=dateIdx, quote = data[0].quote if leftFill is None else leftFill))
        elif thisQuote.timestamp < dateIdx:
            # The current timescale point is past the last data point — use rightFill.
            result.append(QuoteHistoryItem.model_construct(timestamp=dateIdx, quote = data[-1].quote if rightFill is None else rightFill))
        elif quoteIdx == 0:
            ...
            result.append(QuoteHistoryItem.model_construct(timestamp=dateIdx, quote=data[0].quote))
        else:
            ...
            result.append(QuoteHistoryItem.model_construct(timestamp=dateIdx, quote=linearQuote))
